...
class ArrCountTest extends TestCase
{
    ...
    {
        $arr = new Arr(['foo.bar' => 'baz', 'qux.corge' => 'grault', 'gradly.waldo' => ' fred']);
     ...
     */
    public function count_method_reutrns_correct_value_when_simple_or_composite_key()
    {
        $arr = new Arr(['foo' => 'bar', 'baz' => ['qux' => 'corge', 'grault' => 'gradly'], 'waldo' => []]);

        $this->assertEquals(1, $arr->count('foo'));
        $this->assertEquals(2, $arr->count('baz'));
        $this->assertEquals(0, $arr->count('waldo'));
    }
}
